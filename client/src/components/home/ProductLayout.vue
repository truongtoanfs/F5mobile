    ...
    <div class="pb-2">
        <div class="py-5 flex flex-col xl:flex-row xl:justify-between xl:items-center xl:py-1">
            <div class="flex flex-col items-center xl:flex-row">
                ...
                <router-link :to="productSeriesLink" class="text-lg py-0.5 px-3 bg-f5-primary text-white hover:text-gray-100 rounded mt-2.5 xl:mt-0 xl:ml-4">Xem thêm</router-link>
            </div>
            <div class="hidden capitalize text-base font-medium mt-4 mb-2 md:flex md:items-center md:justify-center  xl:text-white xl:font-normal xl:uppercase">
                <router-link v-for="linkObject in productLinksList" :key="linkObject.id"  :to="linkObject.typePath" class="px-4 xl:bg-gray-500 xl:bg-opacity-70 xl:hover:bg-opacity-100 xl:rounded-md xl:ml-1.5 xl:py-0.5 xl:px-2">{{ linkObject.typeName }}</router-link>
            </div>
        </div>
        <div class="grid grid-cols-2 gap-3 xl:grid-cols-5">
            <template v-for="(product, index) in productsList" :key="product.id">
                <div v-if="index < 10" class="bg-white px-2 py-3 rounded-lg text-f5-primary">
                    ...
                    <div class="font-medium text-lg leading-5 flex items-center justify-center flex-wrap mt-1 md:mt-3.5 md:mb-1 lg:mt-2">{{ formatNumberToLocal(product.currentPrice) }} <span class="text-base text-gray-500 line-through mx-0.5 md:mx-1.5">{{ formatNumberToLocal(product.oldPrice) }}đ</span></div>
                </div>
            </template>
        </div>
    </div>
...
export default {
    ...
    setup(props) {
        const productSeriesObject = navList.find(item => item.categoryName === props.layoutTitle.toLowerCase());
        const productSeriesLink = productSeriesObject.categoryPath;
        const productLinksList = productSeriesObject.categoryList;
